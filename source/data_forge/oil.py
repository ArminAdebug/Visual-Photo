...
def main(Image, radius: int):  # --> RGB, [x, y, c]
    ...
    for x in np.arange(1, array_w):
        for y in np.arange(1, array_h):
            ...
            del average_pixels
            most_frequent = sorted(colors.items(), key=lambda x: x[1])[0]
            print("most_frequent :", most_frequent)
            colors.clear()
                
            array_out[x, y] = ((most_frequent[0] + (average_pixels[0] * 0.3)) / 2,
                               (most_frequent[1] + (average_pixels[1] * 0.3)) / 2,
                               (most_frequent[2] + (average_pixels[2] * 0.3)) / 2
                               ) # average

    return array_out


path = r"C:\Users\Armin\Documents\Code\Python\Portfolio\image test folder\test2.png"
PIL_image = Image.open(path)

out_image = Image.fromarray(main(PIL_image, 3))
print("finished")

out_image.save(r"C:\Users\Armin\Documents\Code\Python\Portfolio\image test folder\out.png")
